	...
	circle_texture_(nullptr)
{
	...
	const Point<NumType> center = { static_cast<NumType>(constants::screen_width / 2), static_cast<NumType>(constants::screen_height / 2) };
	const NumType radius = 100;
	circle_texture_ = std::make_unique<CircleTexture<NumType>>(renderer_, center, radius, color);
}
