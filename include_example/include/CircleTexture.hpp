...
public:
	CircleTexture(SDL_Renderer* renderer, const Point<T>& center, T radius, const SDL_Color& color) : 
	renderer_(renderer), 
	center_(center), 
	radius_(radius), 
	color_(color), 
	...
	texture_(nullptr)
    {
        bbox_.x = center_.x_ - radius_;
        bbox_.y = center_.y_ - radius_;
        bbox_.w = 2 * radius_;
        bbox_.h = 2 * radius_;
    ...
    void MoveTo(const Point<T>& new_center)
    {
        center_ = new_center;
        bbox_.x = center_.x_ - radius_;
        bbox_.y = center_.y_ - radius_;
